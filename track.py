    ...
    for frame_idx, (path, im, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                cv2.putText(im0, f"{n} {names[int(c)]}{'s' * (n > 1)}", (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2,(0, 0, 255), 2)   # 只有一个类别就用这个
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if id in dict_id.keys():
                            ...
                            id = new_id
                            new_id += 1
                            #cv2.putText(im0, f"{n} {names[int(c)]}{'new_id' * (n > 1)}", (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2,(0, 0, 255), 2)   # 只有一个类别就用这个
...
def parse_opt(X):
    ...
    parser.add_argument('--source', type=str, default=X,help='file/dir/URL/glob, 0 for webcam')  
